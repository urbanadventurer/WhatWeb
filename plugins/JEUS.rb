...
def passive
	...
	m << { :version=>@meta["server"].scan(/^Jeus WebContainer\/([\d\.]+)$/) } if @meta["server"] =~ /^Jeus WebContainer\/([\d\.]+)$/
	m << { :version=>@meta["server"].scan(/^Jeus WebContainer\/JEUS (.+)$/) } if @meta["server"] =~ /^Jeus WebContainer\/JEUS (.+)$/
