...
matches [
	...
	# Name :
	{ :version=>/<title>bebo.com - Profile from ([^&]*) &lt;[^&]*&gt;<\/title>/, :regexp_offset=>0 },

	# Username :
	{ :model=>/<title>bebo.com - Profile from [^&]* &lt;([^&]*)&gt;<\/title>/, :regexp_offset=>0 },

	# Gender :
	{ :string=>/DynamicValues = \{"FacebookConnect":\{"appId":"[0-9]+"\},"Viximo":\{"memberLocale":"[^\"]*","creatorUid":"[0-9]+","creatorName":"[^\"]*","memberCountry":"[^\"]*","creatorGender":"([^\"]*)"/, :regexp_offset=>0 },

	# Country :
	{ :string=>/DynamicValues = \{"FacebookConnect":\{"appId":"[0-9]+"\},"Viximo":\{"memberLocale":"[^\"]*","creatorUid":"[0-9]+","creatorName":"[^\"]*","memberCountry":"([^\"]*)","creatorGender":"[^\"]*"/, :regexp_offset=>0 },

	# Friends :
	{ :filepath=>/<span class="thumb-label"><a href="\/Profile\.jsp\?MemberId=[0-9]+" title="([^\"]*)">[^\<]*<\/a><\/span>/, :regexp_offset=>0 },

	# Number of Friends :
	{ :string=>/<li class="first">[0-9]+ of <a href="\/FriendList\.jsp\?MemberId=[0-9]+">([^<]*)<\/a><\/li>/, :regexp_offset=>0 },
