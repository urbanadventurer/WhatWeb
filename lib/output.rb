...
#          @f.puts "\tCategory   : " + Plugin.registered_plugins[plugin_name].category.first unless Plugin.registered_plugins[plugin_name].category.nil?
          ...
          description[1..-1].each { |line| @f.puts "\t" + " " * 13 + line }

          unless Plugin.registered_plugins[plugin_name].website.nil?
            @f.puts "\tWebsite    : #{Plugin.registered_plugins[plugin_name].website}"
          end
...
class OutputSQL < Output
  ...
  def create_tables
    ...
    @f.puts "CREATE TABLE targets (target_id int NOT NULL AUTO_INCREMENT, target varchar(255) NOT NULL, status varchar(10),PRIMARY KEY (target_id), UNIQUE (target, status) );"
  ...
  def out(target, status, results)
    ...
    results.each do |plugin_name, plugin_results|
      ...
      unless plugin_results.empty?
        ...
        thisplugin[:certainty] = certainty if !certainty.nil? and certainty != 100
        # empty arrays
